...
The right-side file keeps the same title but changes the order of later
sections. This is useful for checking how a larger markdown document behaves
when entire paragraphs move and are partially rewritten.
...
while scroll positions remain synchronized. The refined wording adds a line
about the refresh button and the sidebar filters for extra token changes.

## Section B

The sidebar summarizes changed files by folder and status. This paragraph moves
below the unified-view notes on the right side. The wording is also adjusted
enough to force both line-level and token-level differences during the compare.
