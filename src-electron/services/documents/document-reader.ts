...
export function encodeDocument(contents: string, format: DocumentFormat): Uint8Array {
  ...
  switch (format.encoding) {
    ...
    case 'windows1252':
      return encodeSingleByte(normalized, WINDOWS_1252_REVERSE, 'Windows-1252')
    case 'latin1':
      return encodeSingleByte(normalized, null, 'ISO-8859-1')
  }
}
...
function encodeSingleByte(
  contents: string,
  extended: Map<number, number> | null,
  label: string,
) {
  const bytes: number[] = []
  for (const character of contents) {
    const codePoint = character.codePointAt(0)!
    const mapped = extended?.get(codePoint)
    if (mapped !== undefined) {
      bytes.push(mapped)
      continue
    }
    const directlyRepresentable = extended
      ? codePoint <= 0x7f || (codePoint >= 0xa0 && codePoint <= 0xff)
      : codePoint <= 0xff
    if (!directlyRepresentable) {
      throw new UnsupportedDocumentEncodingError(`${label} cannot represent U+${codePoint.toString(16).toUpperCase()}.`)
    }
