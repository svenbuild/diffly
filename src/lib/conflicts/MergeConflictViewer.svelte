<script lang="ts">
  ...
  function resolveAll(kind: 'current' | 'incoming' | 'both') {
    workspaceConflictController.updateDraft(resolveAllMarkers($conflictStore.draft, kind))
    renderedDraft = ''
    renderUnresolved()
  }
  ...
  {:else if $conflictStore.document}
    ...
    {:else if manual || !$conflictStore.document.markerContents}
      <textarea
        ...
        value={$conflictStore.draft}
        on:input={(event) => workspaceConflictController.updateDraft(event.currentTarget.value)}
      ></textarea>
    ...
    <footer>
      <button class="secondary" type="button" on:click={() => workspaceConflictController.saveDraft()}>Save draft</button>
...
<style>
  ...
  textarea { min-width: 0; min-height: 0; resize: none; border: 0; border-radius: 0; padding: 12px; background: var(--editor-bg); color: var(--strong-text); font: 13px/1.5 var(--code); }
